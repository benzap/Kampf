...
test_case(
	...
	function()
		...
		local component = kf.GraphicsComponent("graphical")
		
		--get it's current name
		test_assert("getName", component:getName() == "graphical");
		...
		test_assert("toString",
					tostring(component) == "Component:GRAPHICS:ben");
		
		--get the family it belongs to
		test_assert("getFamily", component:getFamily() == "GRAPHICS")
