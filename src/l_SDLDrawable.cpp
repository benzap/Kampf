...
static int l_SDLDrawable_setColorKey(lua_State *L) {
    auto sdlDrawable = lua_to_sdldrawable(L, 1);
    unsigned int red_color = luaL_checkint(L, 2);
    unsigned int green_color = luaL_checkint(L, 3);
    unsigned int blue_color = luaL_checkint(L, 4);

    sdlDrawable->setColorKey(red_color,
			     green_color,
			     blue_color);
...
static const struct luaL_Reg l_SDL_Drawable [] = {
    ...
    {"setColorKey", l_SDLDrawable_setColorKey},
